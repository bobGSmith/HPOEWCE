...
#' @param results The dataframe of subset of RD EWCE results to be plotted in the cell select app.
...
#' @examples
#' \dontrun{
#' add_hpo_termid_col(results,phenotype_to_genes,hpo)}
#' @returns The subset of ewce result data frame with a HPO Id column added.
#' @export
add_hpo_termid_col = function(results, phenotype_to_genes, hpo) {
  HPOtermID = c()
  ValidTerm = c()
  ...
  results$HPO_term_Id =HPOtermID
  results$HPO_term_valid = ValidTerm
  return(results)
}
